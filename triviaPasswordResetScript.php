...
$newPass2 = $_POST[newPassword2];
		...
		$saltOldPass = md5($salt1 . $oldPass . $salt2); 
		$saltNewPass = md5($salt1 . $newPass1 . $salt2); 


	if ($newPass1 != $newPass2) {
		echo "Error... New Passwords do not match<br>";
		echo "<a href=triviaCellReset.php>Please try again</a>";
		exit;
	}
	else {
...
//			$pass = $row['user_password'];
			...
			$row = mysql_fetch_array($result);
				...
				if ($saltOldPass == $pass) {
					$result = mysql_query("UPDATE user SET user_password = '$saltNewPass' WHERE user_cell_num = '$cell'") 
						or die(mysql_error());
					print "Your password has been changed.  <a href=triviaRegisterSuccess.php>Please log in again.</a>";
				}
				else {
