...
<div id="contentDiv">
    ...
    <div id="sendText">
		...
		<div id="textPreferences">
            ...
            <br />
			<input type="radio" name="sex" value="SELECT u.user_cell_num FROM user u JOIN site_opt so on u.userID = so.userID JOIN preference_match pm on u.userID = pm.userID Where so.siteID = 00075 and pm.answer = 'male'">Men<br />
			<input type="radio" name="sex" value="SELECT u.user_cell_num FROM user u JOIN site_opt so on u.userID = so.userID JOIN preference_match pm on u.userID = pm.userID Where so.siteID = 00075 and pm.answer = 'female'">Women<br />
            <input type="radio" name="sex" value="SELECT u.user_cell_num FROM user u Left JOIN site_opt so on u.userID = so.userID where so.siteID = '$siteID'" checked="checked">Everyone<br />
