...
if ($remove != "") {
	$result = mysql_query("DELETE FROM site_ads WHERE ad_num='1'") 
		or die(mysql_error());
		print ("<script>location.href='manageAds.php'</script>");
}
else {

$result = mysql_query("SELECT * FROM site_ads WHERE ad_num='1'")
	or die(mysql_error());
	...
	if ($status == 'Pending'){
		print "<div id=contentDiv>";
			print "<div id=triviaBody>";
				...
				print "<form name=confirmRemove ACTION=updateAd1.php METHOD=POST>";
		...
		$result = mysql_query("UPDATE site_ads SET ad_num='null' WHERE ad_num='1'") 
			or die(mysql_error());
			print ("<script>location.href='manageAds.php'</script>");
	}
}
